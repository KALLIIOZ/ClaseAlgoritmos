...
def on_select_file():
    ...
    if file_path:
        ...
        if content:
            global initial_size, freq_table, huffman_tree
            ...
            output_file_path = "./Gullivers_Travel_compressed.bin"
            with open(output_file_path, 'wb') as output_file:
                output_file.write(int(encoded_data, 2).to_bytes((len(encoded_data) + 7) // 8, byteorder='big'))
                
def on_select_encode_file():
        file_path = filedialog.askopenfilename(title="Seleccionar archivo")
        content = read_file_b(file_path)
        if content:
            decoded_output_file_path = "./Gullivers_Travel_decompressed.txt"
            decode_and_save(file_path, decoded_output_file_path, huffman_tree)
...
estilo_boton = {'bg': '#4CAF50', 'fg': 'white', 'font': ('Arial', 12)}
estilo_etiqueta = {'font': ('Arial', 14), 'bg': '#f0f0f0', 'fg': '#333'}

etiqueta = tk.Label(ventana, text="Selecciona un archivo", **estilo_etiqueta)
etiqueta.pack(pady=10)

boton_seleccionar = tk.Button(ventana, text="codificar", command=on_select_file, **estilo_boton)
boton_seleccionar.pack(pady=10)
boton_seleccionar = tk.Button(ventana, text="decodificar", command=on_select_encode_file, **estilo_boton)
boton_seleccionar.pack(pady=10)

boton_salir = tk.Button(ventana, text="Salir", command=ventana.quit, **estilo_boton)
...
initial_size_label = tk.Label(ventana, text="")
initial_size_label.pack()
encoded_size_label = tk.Label(ventana, text="")
